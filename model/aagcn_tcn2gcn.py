...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, tcn_A, kernel_size=9, stride=1):
        super(unit_tcn, self).__init__()
        ...
        bn_init(self.bn, 1)
        self.tcn_A = nn.Parameter(tcn_A)


    def forward(self, x):
        # N, C, T, V = x.size()
        # self.tcn_A = self.tcn_A.cuda(x.get_device())
        # print(self.tcn_A)
        x = self.conv(x)
        x = torch.matmul(x, self.tcn_A)
        x = self.bn(x)
        return x
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, tcn_A, stride=1, residual=True, adaptive=True, attention=True):
        super(TCN_GCN_unit, self).__init__()
        self.gcn1 = unit_gcn(in_channels, out_channels, A, adaptive=adaptive, attention=attention)
        self.tcn1 = unit_tcn(out_channels, out_channels, tcn_A, stride=stride)
        self.relu = nn.ReLU(inplace=True)
        self.tcn_A = tcn_A
        # if attention:
        # self.alpha = nn.Parameter(torch.zeros(1))
        ...
        if not residual:
            ...
            self.residual = unit_tcn(in_channels, out_channels, tcn_A, kernel_size=1, stride=stride)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        A = self.graph.A
        tcn_A = self.graph.tcn_A
        self.num_class = num_class

        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point)

        self.l1 = TCN_GCN_unit(3, 64, A, tcn_A, residual=False, adaptive=adaptive, attention=attention)
        self.l2 = TCN_GCN_unit(64, 64, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l3 = TCN_GCN_unit(64, 64, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l4 = TCN_GCN_unit(64, 64, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l5 = TCN_GCN_unit(64, 128, A, tcn_A, stride=2, adaptive=adaptive, attention=attention)
        self.l6 = TCN_GCN_unit(128, 128, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l7 = TCN_GCN_unit(128, 128, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l8 = TCN_GCN_unit(128, 256, A, tcn_A, stride=2, adaptive=adaptive, attention=attention)
        self.l9 = TCN_GCN_unit(256, 256, A, tcn_A, adaptive=adaptive, attention=attention)
        self.l10 = TCN_GCN_unit(256, 256, A, tcn_A, adaptive=adaptive, attention=attention)
