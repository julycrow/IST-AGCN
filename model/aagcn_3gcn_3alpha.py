...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, A2, A3, coff_embedding=4, num_subset=3, adaptive=True, attention=True):
        ...
        num_jpts = A.shape[-1]
        num_jpts2 = A2.shape[-1]
        num_jpts3 = A3.shape[-1]
        ...
        if adaptive:
            ...
            self.PA2 = nn.Parameter(torch.from_numpy(A2.astype(np.float32)))
            self.PA3 = nn.Parameter(torch.from_numpy(A3.astype(np.float32)))
