...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(5, 1), padding=(int((5 - 1) / 2), 0),
                              stride=(stride, 1))
        self.conv2 = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                               stride=(stride, 1))
        self.conv3 = nn.Conv2d(in_channels, out_channels, kernel_size=(13, 1), padding=(int((13 - 1) / 2), 0),
                               stride=(stride, 1))
    ...
    def forward(self, x):
        x = self.conv(x) * self.beta[0] + self.conv2(x) * self.beta[1] + self.conv3(x) * self.beta[2]
        x = self.bn(x)
    ...
    def forward(self, x):
        ...
        if self.adaptive:
            ...
            for i in range(self.num_subset):
                ...
                x1 = (A[i] + x1 * self.alpha) * self.gamma1  # A[i] = B,alpha = C
                x1_2 = (A[i] ** 2 + x1 * self.alpha) * self.gamma2
                x1_3 = (A[i] ** 3 + x1 * self.alpha) * self.gamma3
                x2 = x.view(N, C * T, V)
