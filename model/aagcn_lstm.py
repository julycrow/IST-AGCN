...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        # conv_init(self.conv)
        if stride == 1:
            self.lstm = nn.LSTM(input_size=in_channels*25, hidden_size=out_channels*25, num_layers=1)
        elif stride == 2:
            self.lstm = nn.LSTM(input_size=in_channels*25, hidden_size=out_channels*25, num_layers=1)
        # nn.init.kaiming_normal_(self.lstm.weight, mode='fan_out')
        bn_init(self.bn, 1)

    def forward(self, x):
        N, C, T, V = x.size()
        x = x.permute(2, 0, 1, 3).contiguous().view(T, N, C * V)
        x = self.lstm(x)[0]
        x = x.view(N, -1, T, V)
        x = self.bn(x)
        return x
